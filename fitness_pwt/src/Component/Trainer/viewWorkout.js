...
export default class viewWorkout extends Component {
    ...
    componentDidMount() {
        fetch('http://localhost:3004/api/v1/users?userName='+this.props.location.query.userName, {
            headers: authHeader()
        })
            .then(response => response.json())
            .then(data => {
                var works=data[0].workouts;
                this.setState({ Workouts: works});
                console.log(data[0].workouts);
            });
    }
    render() {
